...
class LoginAuth
{
    ...
    public function handle($request, Closure $next)
    {
        ...
        }elseif($result->status_id == 0){
            $code = 403;
            $response = [
                'code' => $code,
                //    'success' => false,
                'message' => Lang::get('custom_message.user_block'),
                'data' => [['auth' => Lang::get('custom_message.user_block')]],
            ];
            return response()->json($response, $code);
        }
        
        $request['user_id'] = $result->id;
        $request['user_type'] = $result->type_id;
        $request['call_mode'] = 'web';
        return $next($request);
    }
}
